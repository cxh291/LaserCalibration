...
public:
	...
	void SetNebRadius(double neb_radius);
public slots:
...
private:
	...
	std::vector<CCVector3> pickPoints;
private:
	...
	int extractLinear(const std::vector<Eigen::Vector3d> &seeds, std::vector<int> &index_list, double length);
	bool isOnLinear(CCVector3 P, CCVector3 x1, CCVector3 x2);

private slots:
	void itemPickedSlot(ccHObject* entity, unsigned subEntityID, int x, int y, const CCVector3& P);
	void itemPickedLinearSlot(ccHObject* entity, unsigned subEntityID, int x, int y, const CCVector3& P);
